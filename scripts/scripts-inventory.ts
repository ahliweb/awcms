...
export function renderInventory(rows: readonly InventoryRow[]): string {
  ...
  return [
    ...
    "",
    `${rows.length} targets run a file in \`scripts/\`; ${gated} of them are`,
...
async function main(): Promise<void> {
  ...
  if (mode === "generate") {
    ...
    console.log(
      `Updated: ${README_PATH} — the script inventory block was regenerated.`
    );
    return;
  }
  ...
  if (JSON.stringify(present) !== JSON.stringify(expected)) {
    failures.push(
      `${README_PATH} does not match a fresh render from package.json ` +
    ...
    // pertama kali.
    failures.push(
      `${README_PATH}: the rows match but the generated block does NOT — ` +
  ...
  for (const target of falseAbsences) {
    failures.push(
      `\`${target}\` is listed under §Deferred as absent, but it EXISTS ` +
        "terdaftar di package.json. Klaim negatif menua ke arah berbahaya: " +
        "its reader will go and build a duplicate."
    );
  }

  if (failures.length === 0) {
    console.log(
      `scripts:inventory:check OK — the ${README_PATH} inventory block matches, ` +
        "and no target that exists is claimed to be missing."
    );
    return;
  }

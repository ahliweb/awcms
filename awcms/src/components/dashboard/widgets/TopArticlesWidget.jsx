    ...
    return (
        <Card className="bg-white/60 dark:bg-slate-800/60 backdrop-blur-xl rounded-2xl border-white/40 dark:border-slate-700/40 shadow-sm hover:shadow-lg transition-shadow duration-300">
            <CardHeader className="flex flex-row items-center justify-between pb-2 space-y-0">
                ...
                <Link to="/cmspanel/articles" className="text-sm text-indigo-600 dark:text-indigo-400 hover:text-indigo-800 dark:hover:text-indigo-300 font-medium flex items-center gap-1 group px-3 py-1.5 rounded-lg hover:bg-indigo-50 dark:hover:bg-indigo-900/30 transition-colors">
            ...
            <CardContent className="space-y-3 pt-4">
                {loading ? (
                    ...
                    data.map((article, i) => (
                        <div key={i} className="group flex items-center justify-between p-4 bg-white/40 dark:bg-slate-700/40 rounded-xl border border-white/20 dark:border-slate-600/20 hover:bg-white/80 dark:hover:bg-slate-700/80 hover:border-indigo-100 dark:hover:border-indigo-800 transition-all duration-200">
                            <div className="flex flex-col gap-1">
                                ...
                                <span className="text-xs text-slate-500 dark:text-slate-400">Published in {article.category || 'General'}</span>
                            </div>
                            <div className="flex items-center gap-4 text-sm">
                                <span className="text-slate-500 dark:text-slate-400 font-medium bg-slate-100/50 dark:bg-slate-600/50 px-2 py-1 rounded-md">{article.views || 0} views</span>
                    ...
                    <div className="text-center py-10 bg-slate-50/50 dark:bg-slate-800/50 rounded-xl border border-dashed border-slate-200 dark:border-slate-700">
                        <p className="text-slate-400 dark:text-slate-500">No articles found.</p>
                    </div>
                )}
            </CardContent>
        </Card>

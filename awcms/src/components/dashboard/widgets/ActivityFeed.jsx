        ...
        <div className="space-y-4">
          {activities && activities.length > 0 ? (
            activities.map((activity, index) => (
              <div key={index} className="flex items-start gap-4 rounded-[1.1rem] border border-slate-200/70 bg-white/74 p-3.5 shadow-sm transition-colors hover:border-slate-300/70 hover:bg-white dark:border-slate-700/60 dark:bg-slate-800/40 dark:hover:border-slate-600/80">
                <div className="shrink-0 rounded-full border border-sky-200/70 bg-sky-50/90 p-2 dark:border-sky-500/20 dark:bg-sky-500/10">
                  <FileEdit className="w-4 h-4 text-sky-700 dark:text-sky-300" />
                </div>

...
const CATALOG_SELECT = `
  ...
  invalidated_by_catalog_version,
  restored_by_catalog_version,
  activated_at,
  deactivated_at,
  created_at,
  updated_at,
...
const buildCatalogRecord = (row) => {
  ...
  return {
    ...
    runtime_mode: catalog?.runtime_mode || manifest?.runtime_mode || 'trusted',
    validation_status: row.validation_status || catalog?.validation_status || 'valid',
    validation_summary: row.validation_summary || catalog?.validation_summary || {},
    catalog_validation_status: catalog?.validation_status || 'valid',
    catalog_validation_summary: catalog?.validation_summary || {},
    last_validated_at: catalog?.last_validated_at || null,
    last_invalidated_at: catalog?.last_invalidated_at || null,
    config: row.config || {},
    rollout: row.rollout || {},
    ...
    desired_activation_state: row.desired_activation_state || (row.activation_state === 'active' ? 'active' : 'inactive'),
    auto_deactivated_at: row.auto_deactivated_at || null,
    auto_restored_at: row.auto_restored_at || null,
    deactivation_reason_category: row.deactivation_reason_category || null,
    invalidated_by_catalog_version: row.invalidated_by_catalog_version || null,
    restored_by_catalog_version: row.restored_by_catalog_version || null,
...
const buildLegacyRecord = (row) => {
  ...
  return {
    ...
    runtime_mode: manifest.runtime_mode || 'trusted',
    config: row.config || {},
    rollout: {},
    created_by: null,
    updated_by: null,
    ...
    validation_summary: {},
    catalog_validation_status: 'valid',
    ...
    desired_activation_state: row.is_active ? 'active' : 'inactive',
    auto_deactivated_at: null,
    auto_restored_at: null,
    deactivation_reason_category: null,
    invalidated_by_catalog_version: null,

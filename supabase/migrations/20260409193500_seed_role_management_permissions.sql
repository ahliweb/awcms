-- Seed missing canonical tenant.role.* management permissions.

WITH permission_seed(name, resource, action, description) AS (
  VALUES
    ('tenant.role.create', 'roles', 'create', 'Create roles'),
    ('tenant.role.update', 'roles', 'update', 'Update roles'),
    ('tenant.role.delete', 'roles', 'delete', 'Delete roles')

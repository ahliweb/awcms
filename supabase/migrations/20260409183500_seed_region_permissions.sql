-- Seed canonical tenant.region.* permissions for regional hierarchy management.

WITH permission_seed(name, resource, action, description) AS (
  VALUES
    ('tenant.region.read', 'regions', 'read', 'Read regions'),
    ('tenant.region.create', 'regions', 'create', 'Create regions'),
    ('tenant.region.update', 'regions', 'update', 'Update regions'),
    ('tenant.region.delete', 'regions', 'delete', 'Delete regions')
  ...
  JOIN effective_permissions ep ON (
    ...
    OR (
      ...
      AND ep.name IN (
        'tenant.region.read',
        'tenant.region.create',
        'tenant.region.update',
        'tenant.region.delete'
      )
    )
    OR (
    ...
    OR (
      tr.name = 'internship'
      AND ep.name IN ('tenant.region.read')
    )
  )
)

...
BEGIN
  ...
  ELSE
    WITH restorable AS (
      SELECT id, tenant_id, installed_version, activation_state
      FROM public.tenant_extensions current_extension
      WHERE current_extension.catalog_id = v_catalog_id
        AND current_extension.deleted_at IS NULL
      ...
      WHERE target.id = source.id
      RETURNING target.id, target.tenant_id, source.installed_version, source.activation_state
    ), audit_rows AS (
      INSERT INTO public.extension_lifecycle_audit (
        tenant_id,
        catalog_id,
      ...
      SELECT
        ...
        jsonb_build_object(
          ...
          'invalidated_by_catalog_version', public.tenant_extensions.invalidated_by_catalog_version,
          'restored_by_catalog_version', p_version
        )
      FROM updated
      JOIN public.tenant_extensions ON public.tenant_extensions.id = updated.id
      RETURNING 1
    )

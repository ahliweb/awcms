...
BEGIN
  proc_oid := to_regprocedure('extensions.index_advisor(text)');

  IF proc_oid IS NULL THEN
    RAISE NOTICE 'extensions.index_advisor(text) not found, skipping patch.';
    RETURN;
  END IF;

  SELECT (pg_get_userbyid(proowner) = current_user)
  INTO is_owner
  FROM pg_proc
  WHERE oid = proc_oid;
  ...
  IF is_owner OR is_super THEN
    ...
    $sql$;
  ELSE
    RAISE NOTICE 'Skipping extensions.index_advisor patch due to insufficient privileges.';
  END IF;
END $$;

...
- **Cache (`application/policy-cache.ts`).** Kebijakan aktif dikompilasi sekali per tenant, di-cache in-process **tenant-keyed**, di-invalidasi **deterministik** oleh setiap create/update/enable/disable **dari kedua permukaan** (`invalidatePolicyCache` **setelah commit**). Load selalu di `withTenant` (RLS + `awcms_app`). Batasan: invalidasi per-proses (multi-instance butuh LISTEN/NOTIFY/TTL).
- **Dua permukaan authoring.** Baru: `GET/POST /api/v1/access/policies`, `GET/PUT /api/v1/access/policies/{id}`, `POST /api/v1/access/policies/{id}/{enable,disable}` (guard `identity_access.abac_policies.{read,configure}`, DSL penuh, audited) + `POST /api/v1/access/policies/simulate` (guard `.analyze`, read-only, audit tanpa decision log; subjek asing juga butuh `access_control.read`) + `POST /api/v1/access/evaluate` (mencerminkan keputusan nyata). Permission di-seed `sql/032`, kolom DSL `sql/031`. Lama (#171): `/api/v1/abac/policies` flat — dipertahankan, kini **juga** menginvalidasi cache.
- **Contoh (bukan base).** Base **tidak** menyertakan kebijakan domain. Lima contoh ERP ada di `fixtures/abac-example-policies.json` untuk di-author aplikasi turunan lewat API.
...
- **Rule descriptor code-only, kontribusi via composition seam (#178).** `SoDRuleDescriptor` (`_shared/module-contract.ts`) dideklarasikan `module.ts` modul pemilik (`sodRules`) — pasangan `conflictingPermissionKeys` (≥2), `scopeApplicability` (`same_scope_only`/`global_within_tenant`/`any`), `severity`, `exceptionPolicy`. **Base tidak men-hardcode rule domain**; rule mengalir lewat `listModules()` dari aplikasi turunan. Contoh ilustratif (≥5) hidup di fixture `tests/fixtures/derived-application-example/`, **bukan** modul base. Gate `bun run identity-access:sod-registry:check` (`domain/sod-rule-registry.ts`) memvalidasi registry ter-compose; drift (duplicate ruleKey/owner mismatch) → CI merah.

...
| `POST /api/v1/omes/servers/{id}/enrollment-challenges` | `enrollments.manage`      | Mints a one-time challenge; the raw value is returned exactly once and never persisted (sql/157 stores only its sha256 hash). Idempotency-Key required, rate-limited, audited.                                      |
...
Every mutation requires `Idempotency-Key` and replays the stored response on a retry with the same key+payload (shared `awcms_idempotency_keys` store, `modules/_shared/idempotency.ts`) — `409 IDEMPOTENCY_CONFLICT` for the same key with a different payload. Registration, enrollment-challenge issuance, operation submission, and backup restore are additionally rate-limited per authenticated actor (`checkSharedRateLimit`). Every jsonb evidence field (`target`/`payload`/`result`/`parameters`/`desiredState`/`observedState`/`checks`/`manifest`/`evidence`) is passed through `redactSensitiveAttributes` before leaving this module, as defense-in-depth on top of whatever wrote it.

## The `omes#197` integration seam

Contract shape/state-machine validation against the vendored OMES contracts (`validateOmesContract`, `scanForRawSecrets`) is owned by a parallel change (ahliweb/omes#197) and is **not** duplicated here. This module's client-facing `validateOperationSubmission`/`validateServerRegistrationInput` (`domain/operations.ts`, `domain/server-registration.ts`) validate the AWCMS-facing (session-authenticated) request shape only — deliberately not the OMES wire schema, whose `tenant_id`/`correlation_id`/`idempotency_key`/`actor`/`permission` fields are derived server-side, never accepted from a browser. Building that OMES-facing envelope is the pull worker's job (`ahliweb/omes#199`). Once #197 lands, its contract validator is the single place to plug in before this module's requests are ever forwarded to OMES.

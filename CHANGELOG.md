...
- Templates: added Phase 15 update-linked workflows for reusable sections so all still-linked usages of a section can be refreshed from the current source in one pass while preserving linkage, with focused manager and helper coverage.
- Templates: added Phase 16 reusable-section revision history with mirrored `reusable_section_revisions` schema, save-time revision snapshots, manager-side revision visibility, and focused revision-surface validation.
...
- Templates: added Phase 23 centralized reusable section approval queue/history with a dedicated Templates tab for approvers to review pending and historical bulk action requests across all sections.
...
- Visual Builder / Templates: still-linked reusable section instances can now be refreshed from the current source in bulk through the new `Update Linked` workflow without detaching them.
...
- Visual Builder / Templates: reusable section approval workflows now also provide a centralized cross-section queue/history view for approvers inside the Templates workspace.
...
- Versioning: Prepared the next patch release by bumping the maintained core workspaces to `4.3.1` in line with the documented independent-versioning strategy for `awcms`, `awcms-public/primary`, `awcms-public/smandapbun`, and `awcms-mcp`.
- Admin UI: Realigned `awcms/src/lib/version.js` with the canonical admin package manifest so the displayed app version matches the `4.3.1` patch bump.
...
- Architecture: Removed the remaining Coolify-specific deployment-cell references from provisioning docs/helpers and aligned deployment-cell schema/seed usage to Cloudflare + Supabase runtime metadata only.
- Repository Cleanup: Removed the retired `awcms-mobile-java/` workspace, pruned docs-check/link-validator references to its README, and updated the historical changelog note to reference the retired Java mobile client generically.

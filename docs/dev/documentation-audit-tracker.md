...
| DOCSYNC-044 | Medium | Dependency drift remains across maintained workspaces during the 2026-03-12 conflict-resolution pass | In Progress | A low-risk batch plus targeted admin/MCP/public major updates have been applied and validated; the remaining held-major drift is now limited to the intentionally deferred Tailwind `4.2.x` public track |
...
- Updated `docs/modules/TEMPLATE_SYSTEM.md` and `docs/modules/AUDIT_TRAIL.md` so route notes now match the current tab-backed template manager surface, splat-backed visual-editor paths, and the canonical audit route aliasing.
...
- Applied a low-risk dependency batch and revalidated:
  - `awcms`: `npm run lint && npm run build` passed after upgrading `@supabase/supabase-js`, `dompurify`, `framer-motion`, `i18next`, `react-i18next`, and `@vitejs/plugin-react`.
  - `awcms-public/primary`: `npm run check && npm run build` passed after upgrading the current patch/minor Astro/Supabase/TypeScript-ESLint packages.
  - `awcms-mcp`: `npm run lint && npm run build` passed after upgrading the TypeScript-ESLint package family.
- Applied a targeted admin major batch and revalidated:
  ...
  - `awcms`: ESLint 10 now validates after removing the incompatible `eslint-plugin-react` dependency from the flat config and fixing the `no-useless-assignment` findings surfaced by the stricter rule set.
...
- Applied the coupled public Astro major batch and revalidated:
...
- `npm outdated` now shows only the held-major backlog:
  - `awcms-public/primary`: Tailwind `4.2.x`

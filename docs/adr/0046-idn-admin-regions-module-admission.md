...
- **Aktivasi/rollback** adalah aksi admin lewat HTTP (`POST /api/v1/idn-regions/datasets/{id}/activate` dan `.../rollback`): high-risk, ber-`Idempotency-Key`, ter-audit, ABAC-gated. Ini keputusan operasional yang butuh jejak siapa/kapan/kenapa — bukan langkah deployment.

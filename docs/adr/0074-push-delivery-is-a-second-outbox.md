...
Adapter nyata (FCM HTTP v1 dan Web Push/VAPID) serta permukaan HTTP untuk mengelola langganan mendarat di issue-nya sendiri (#466). Sampai itu, satu-satunya adapter adalah `log`, dan `PUSH_PROVIDER` sengaja **belum** menerima nilai `fcm`/`web_push` — menamainya sekarang akan membuat sebuah deployment lolos validasi lalu gagal saat resolve.

Konsekuensi yang perlu disebut jujur: `enqueuePushToRecipients` belum punya pemanggil produksi di PR ini. Itu berbeda dari `broker-adapter-port.ts` yang ditolak di atas — port itu adalah kait registrasi yang, menurut docblock-nya sendiri, tidak bisa bekerja untuk konsumen ber-I/O; ini adalah API tulis sebuah antrean yang bekerja penuh dan diuji, menunggu permukaannya. Perbedaannya nyata, tetapi ia tetap kesenjangan, dan ia dicatat di sini alih-alih dibiarkan ditemukan.

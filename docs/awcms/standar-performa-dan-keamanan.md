...
[`AGENTS.md`](../../AGENTS.md), di 69 ADR, dan di 34 gerbang rantai
...
| A.8.25 Secure development lifecycle      | ADR wajib; changeset per perubahan perilaku; 34 gerbang di rantai `check`                                                        |
...
| C7  | Core Web Vitals: bagian LAB (Opsi D) sudah diukur; bagian RUM masih menunggu pemilik produk                                       | Performa | **SEBAGIAN DITUTUP** — Opsi D mendarat 2026-08-05: `tests/e2e/cwv-lab.e2e.ts` mengukur LCP+CLS `/login` (gerbang `E2E_CWV_LAB` di job `e2e-smoke`, `bun run perf:cwv:lab`); angka lab = detektor regresi, BUKAN p75 lapangan, INP tidak diklaim. Keputusan RUM (Opsi B) tetap milik pemilik produk                                                    | Keputusan pemilik produk atas bagian RUM ADR-0067 — gerbang lab **nol** data pengunjung-nya sudah mendarat dan menyatakan skip saat tidak berjalan           |
...
| C13 | 101 changeset menunggu (1 `major` → rilis berikutnya `v7.0.0`)                                                                    | Standar  | **TERBUKA — penutupan berjalan** (5 Agustus 2026): version PR `v7.0.0` disiapkan; tag + publish menunggu approval environment `release`                                                                                                                                                                                                               | Rilis `v7.0.0`, atau ambang changeset yang memerahkan CI                                                                                                     |

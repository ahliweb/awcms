...
> **Status dokumen:** implementasi awal ada, belum pernah dieksekusi sebagai rilis nyata. Repo `awcms` saat ini di versi `5.0.0` (lompatan manual melanjutkan lini major legacy `v4.6.0`, bukan hasil rilis nyata — lihat [ADR-0024](../adr/0024-semver-numbering-continues-legacy-major-line.md)), belum ada modul ERP. Kedua workflow yang dokumen ini deskripsikan sudah ada (`.github/workflows/changesets.yml`, `.github/workflows/release.yml`, `Dockerfile.production`, `scripts/release-verify.ts`) — dibangun sebagai adaptasi pipeline yang sudah terbukti di basis `awcms-mini`. **Belum pernah dijalankan** terhadap tag nyata sama sekali (belum ada rehearsal/dry-run yang benar-benar dieksekusi di Actions, belum ada `environment: release` dengan required reviewers dikonfigurasi — lihat §Environment approval) — bagian ini dokumen tetap berupa target sampai rehearsal pertama sukses.

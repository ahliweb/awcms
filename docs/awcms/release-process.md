...
>
> **⚠️ Gap hardening yang masih terbuka:** pada rilis `v6.0.0` job `sign-attest-publish` berjalan **tanpa jeda "Waiting for review"** — membuktikan GitHub Environment `release` **belum dikonfigurasi dengan required reviewers**. Approval gate manual (lihat §Environment approval) BELUM aktif; publish/sign/attest saat ini berjalan otomatis begitu tag `v*` di-push. Konfigurasikan required reviewers sebelum mengandalkan gerbang approval untuk rilis production berikutnya.
>

...
> **Status dokumen:** standar target, bukan status implementasi. Repo `awcms` saat ini di versi `5.0.0` (lompatan manual melanjutkan lini major legacy `v4.6.0`, bukan hasil rilis nyata — lihat [ADR-0024](../adr/0024-semver-numbering-continues-legacy-major-line.md)), belum ada modul ERP, belum ada `.github/workflows/release.yml` yang nyata (`changesets.yml` sudah ada). Dokumen ini mengadaptasi pipeline release yang sudah terbukti dan telah dieksekusi (real tags, real image, real signature) di basis `awcms-mini` menjadi **desain pipeline yang wajib diimplementasikan** sebelum rilis publik pertama `awcms` (tag `v5.0.0` + GitHub Release nyata, mengikuti nomor yang sudah diputuskan ADR-0024). Mekanisme (changeset policy, dua workflow, SBOM ganda, keyless signing, provenance) dipertahankan sebagai standar wajib.

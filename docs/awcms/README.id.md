...
Folder ini berisi paket dokumen teknis standar AWCMS — **basis/fondasi** tempat ERP & solusi bisnis dibangun di atasnya ([ADR-0022](../adr/0022-erp-modules-live-in-extension-repos.md)) — diadaptasi dari paket dokumen `docs/awcms-mini/` di repo [awcms-mini](https://github.com/ahliweb/awcms-mini) (basis teknologi: Bun + Astro 7 + PostgreSQL/RLS, modular monolith), disesuaikan untuk skop fondasi yang lebih besar: kontrak kesiapan ERP (keuangan, inventori, procurement, manufaktur, HR/payroll) dan kerangka integrasi solusi bisnis eksternal.

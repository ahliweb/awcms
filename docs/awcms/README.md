...
<!-- i18n-source-hash: sha256:f433b794ca147cc5a6f7f9398dfd12d002915b46741a052d3da758a0fd50c1ae -->

# AWCMS Technical Document Package

This folder holds AWCMS's standard technical document package — a **foundation/base** that ERP & business solutions are built on top of ([ADR-0022](../adr/0022-erp-modules-live-in-extension-repos.md)) — adapted from the `docs/awcms-mini/` document package in the [awcms-mini](https://github.com/ahliweb/awcms-mini) repo (technology base: Bun + Astro 7 + PostgreSQL/RLS, modular monolith), tailored to a broader foundation scope: ERP-readiness contracts (finance, inventory, procurement, manufacturing, HR/payroll) and a framework for integrating external business solutions.
...
This repo is only at the rebuild-foundation stage (see [ADR-0001](../adr/0001-rebuild-on-awcms-foundation-erp-scope.md)) — the current code implementation covers the Sprint 1–2 foundation (`logging`, `tenant-admin`, `profile-identity`, `identity-access`; see [`../ARCHITECTURE.md`](../ARCHITECTURE.md)). Every document in this folder is a **plan/target**, not the current state of the code. Claims of "already live/available/verified" from the awcms-mini source documents (whose own base really has finished being built) should be read as binding targets for future implementation here.

This package now adapts **the entire** technical document set from the awcms-mini reference repo so AWCMS is ready to be developed with the same agent tooling (see also [`.claude/skills/`](../../.claude/skills/README.md) and [`Pedoman_Penggunaan_Agent_Keluarga_AWCMS_v1.0.pdf`](../Pedoman_Penggunaan_Agent_Keluarga_AWCMS_v1.0.pdf)). Documents originating from awcms-mini's CMS products (news portal, social publishing, visitor analytics) or from the reference repo's process (roadmap, SOPs, GitHub issue snapshots) are adapted as **patterns/reference** — map them to this repo's foundation scope when working on the equivalent module, not treated as already-agreed ERP features.
...
| [`15_frontend_architecture_integration.md`](15_frontend_architecture_integration.md)                   | Frontend architecture (Astro SSR, islands, offline-first)                          |
...
| [`derived-application-guide.md`](derived-application-guide.md)                                         | Guide to building a derived application on top of the base                         |
| [`derived-app-pilot-plan.md`](derived-app-pilot-plan.md)                                               | Derived-application pilot plan                                                     |

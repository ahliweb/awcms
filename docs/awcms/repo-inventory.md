...
> **Status dokumen:** standar/template, BUKAN hasil generate nyata — dan **isi body di bawah kini USANG**. Body masih menyatakan "belum ada modul" dan "belum ada migrasi"; itu **tidak lagi benar** — repo `awcms` sekarang punya **4 modul** (`logging`, `tenant-admin`, `profile-identity`, `identity-access`) dan **7 migrasi** (`sql/001`–`007`). Sumber kebenaran keadaan kode saat ini adalah [`../ARCHITECTURE.md`](../ARCHITECTURE.md), bukan berkas ini. Tabel di bawah adalah **struktur target** yang baru akan terisi akurat begitu `scripts/repo-inventory-generate.ts` (`bun run repo:inventory:generate`) diimplementasikan/di-port dari `awcms-mini`. Mekanisme (generated-file, freshness check, sumber data) diadaptasi dari basis `awcms-mini` yang sudah menjalankannya secara nyata.

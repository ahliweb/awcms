...
### OMES worker enrollment/poll/result/heartbeat ingestion (ahliweb/omes#199, ADR-0122)

`POST /api/v1/omes/worker/{enroll,poll,result,heartbeat}` (`src/modules/omes-control/`) are the only routes in this codebase that are **session-UNauthenticated by design** and still reach a tenant-scoped database transaction: the caller is an OMES host pull worker (ADR-0027's outbound-pull architecture — no public privileged listener on the host, no browser-to-host trust), not an AWCMS user.

| Risk                                                                                                     | Mitigation                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| -------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| A stranger polls/submits results/heartbeats for a tenant/server/worker they do not control               | Every request past enrollment is authenticated by an Ed25519 signature (`X-Omes-Worker-Signature`) over a canonical string binding method/path/tenant/server/worker/timestamp/nonce/body-hash, verified against the enrolled worker's stored public key (`domain/worker-identity.ts`, `application/worker-envelope-guard.ts`'s `verifyWorkerEnvelope`, the single chokepoint all three non-enroll routes call before any other side-effecting work). A failure of ANY kind — wrong tenant, unknown worker, revoked identity, bad signature — answers the identical neutral `re-enroll_required`/`rejected` response; none is distinguished in the reply. |
| Replay of a captured, otherwise-valid signed request                                                     | A persisted, atomic nonce store (`awcms_omes_worker_nonces`, unique `(tenant_id, worker_id, nonce)`, consumed via `INSERT ... ON CONFLICT DO NOTHING RETURNING id` — never a read-then-write) plus a bounded timestamp window (5 minutes, both directions) rejects a captured envelope replayed later or presented with a future timestamp.                                                                                                                                                                                                                                                                                                              |
| A challenge is redeemed twice (two workers race the same enrollment challenge)                           | Redemption is a genuine DB-level compare-and-set — `SELECT ... FOR UPDATE` then an `UPDATE` of the same locked row in the same transaction (`application/worker-enrollment-exchange.ts`), so a concurrent second redemption blocks on the row lock and observes `status <> 'pending'` once unblocked. Exercised by a real concurrent-request integration test asserting on the resulting row count, not on the HTTP status alone.                                                                                                                                                                                                                        |
| A stolen/observed enrollment challenge is redeemed by someone who does not hold the matching private key | Enrollment additionally requires a signature over the raw challenge value under the presented public key (`X-Omes-Enrollment-Signature`) — proof of possession, checked before any database work.                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| A worker's self-reported job outcome is presented as independently confirmed success                     | Every `awcms_omes_worker_results` row is stamped `source = 'worker_reported'` / `reconciled = false`; nothing in this issue's scope flips `reconciled` to `true`. A 2xx from `/worker/result` is an acknowledgement of receipt, never a verdict.                                                                                                                                                                                                                                                                                                                                                                                                         |
| A retried/duplicated result submission double-applies a job-state transition                             | Idempotent by `(tenant_id, server_id, idempotency_key)` via the same atomic `INSERT ... ON CONFLICT DO NOTHING RETURNING id` pattern as the nonce store; the job-state update only ever fires for the winning insert. Exercised by a real concurrent-request test asserting exactly one result row and one job-state transition.                                                                                                                                                                                                                                                                                                                         |
| A missing/suppressed heartbeat is read as "server is healthy"                                            | Staleness is a pure function of `now` computed on every READ from `last_heartbeat_at`'s age (`domain/staleness.ts`) — never a stored flag this endpoint could forget to clear. A `decommissioned` server's heartbeat is acknowledged and recorded but never flips its status back to `online`.                                                                                                                                                                                                                                                                                                                                                           |
| Cross-tenant reference via the nonce/results tables                                                      | Composite FK `(tenant_id, worker_id)` on the nonces table / `(tenant_id, server_id, idempotency_key)` on the results table (RI checks run past RLS — GHSA-r7cx-c4jh-cvvw), not a plain single-column FK, plus RLS `FORCE` on both new tables.                                                                                                                                                                                                                                                                                                                                                                                                            |
| Unbounded/abusive request volume against an unauthenticated surface                                      | Bounded request bodies (`readCappedText`, the same streaming size cap `readJsonBody` uses), plus TWO rate-limit layers on every one of the four routes: per-(tenant, worker) as claimed in the body (bounds abuse of one real identity, but is reset by an attacker who simply varies the claimed identity per request — an honestly-documented limitation, not a silent gap) and per-source-IP (`resolveClientIp`, the actual aggregate-volume bound, coarser to tolerate a NAT/shared host running several worker processes).                                                                                                                          |

Known gap, flagged rather than silently worked around: the OMES-side reference pull worker merged for `ahliweb/omes#192` does not yet generate a real Ed25519 keypair or send any signature/nonce/timestamp headers. This module does not relax verification to match that stub.

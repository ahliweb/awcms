...
| **Bypass** — mengubah kebijakan lewat CRUD flat #171 tanpa efek                                   | Permukaan flat `/api/v1/abac/policies` (#171) menulis tabel yang sama, kini **juga** memanggil `invalidatePolicyCache` setelah commit — tidak menjadi jalan mem-bypass evaluator. Footgun terdokumentasi: `deny` flat = applicability wildcard → men-deny SETIAP request; untuk membatasi pakai permukaan DSL. |

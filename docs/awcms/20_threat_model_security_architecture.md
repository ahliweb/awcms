...
| Cross-tenant reference via the nonce/results tables                                                      | Composite FK `(tenant_id, worker_id)` / `(tenant_id, idempotency_key)` (RI checks run past RLS — GHSA-r7cx-c4jh-cvvw), not a plain single-column FK, plus RLS `FORCE` on both new tables.                                                                                                                                                                                                                                                                                                                                                                                                                                                                |

...
<!-- i18n-source-hash: sha256:52d88bf6ab3c84880528f90df1eff9b9cd16ab1e08d223663cb8c4682f145c40 -->
...
### Ingestion enrollment/poll/result/heartbeat worker OMES (ahliweb/omes#199, ADR-0122)

`POST /api/v1/omes/worker/{enroll,poll,result,heartbeat}` (`src/modules/omes-control/`) adalah satu-satunya rute di codebase ini yang **sengaja tidak terautentikasi sesi** namun tetap mencapai transaksi database ber-scope tenant: pemanggilnya adalah pull worker host OMES (arsitektur outbound-pull ADR-0027 — tidak ada listener privileged publik di host, tidak ada kepercayaan browser-ke-host), bukan pengguna AWCMS.

| Risiko                                                                                                     | Mitigasi                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| ---------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Orang asing melakukan poll/mengirim result/heartbeat untuk tenant/server/worker yang tidak dikuasainya     | Setiap request setelah enrollment diautentikasi dengan signature Ed25519 (`X-Omes-Worker-Signature`) atas string kanonik yang mengikat method/path/tenant/server/worker/timestamp/nonce/hash-body, diverifikasi terhadap public key tersimpan milik worker yang sudah enrolled (`domain/worker-identity.ts`, `verifyWorkerEnvelope` di `application/worker-envelope-guard.ts`, satu-satunya chokepoint yang dipanggil ketiga rute non-enroll sebelum pekerjaan ber-efek-samping lain). Kegagalan APA PUN — tenant salah, worker tak dikenal, identitas dicabut, signature salah — menjawab dengan respons netral `re-enroll_required`/`rejected` yang identik; tidak satu pun dibedakan dalam balasannya. |
| Replay dari request bertanda tangan valid yang direkam                                                     | Penyimpanan nonce atomik dan persisten (`awcms_omes_worker_nonces`, unik `(tenant_id, worker_id, nonce)`, dikonsumsi lewat `INSERT ... ON CONFLICT DO NOTHING RETURNING id` — tidak pernah baca-lalu-tulis) ditambah jendela timestamp terbatas (5 menit, kedua arah) menolak amplop yang direkam lalu di-replay kemudian atau disajikan dengan timestamp masa depan.                                                                                                                                                                                                                                                                                                                                     |
| Challenge ditukar dua kali (dua worker berlomba pada challenge enrollment yang sama)                       | Penukaran adalah compare-and-set level-DB sungguhan — `SELECT ... FOR UPDATE` lalu `UPDATE` pada baris terkunci yang sama di transaksi yang sama (`application/worker-enrollment-exchange.ts`), sehingga penukaran kedua yang konkuren terblokir pada row lock dan mengamati `status <> 'pending'` begitu tidak terblokir lagi. Diuji dengan integration test request konkuren sungguhan yang menegaskan pada hitungan baris hasil, bukan hanya status HTTP.                                                                                                                                                                                                                                              |
| Challenge enrollment yang dicuri/diamati ditukar oleh seseorang yang tidak memegang private key yang cocok | Enrollment tambahan mewajibkan signature atas nilai challenge mentah dengan public key yang disajikan (`X-Omes-Enrollment-Signature`) — bukti kepemilikan, diperiksa sebelum pekerjaan database apa pun.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| Hasil job yang dilaporkan sendiri oleh worker disajikan sebagai sukses yang dikonfirmasi independen        | Setiap baris `awcms_omes_worker_results` diberi cap `source = 'worker_reported'` / `reconciled = false`; tidak ada yang di dalam cakupan issue ini yang membalik `reconciled` menjadi `true`. Respons 2xx dari `/worker/result` adalah pengakuan penerimaan, tidak pernah putusan.                                                                                                                                                                                                                                                                                                                                                                                                                        |
| Pengiriman result yang dicoba ulang/diduplikasi menerapkan transisi state job dua kali                     | Idempotent berdasarkan `(tenant_id, server_id, idempotency_key)` lewat pola atomik `INSERT ... ON CONFLICT DO NOTHING RETURNING id` yang sama dengan penyimpanan nonce; update state job hanya pernah terpicu untuk insert yang menang. Diuji dengan test request konkuren sungguhan yang menegaskan tepat satu baris result dan satu transisi state job.                                                                                                                                                                                                                                                                                                                                                 |
| Heartbeat yang hilang/ditekan dibaca sebagai "server sehat"                                                | Staleness adalah fungsi murni dari `now` yang dihitung pada SETIAP BACA dari umur `last_heartbeat_at` (`domain/staleness.ts`) — tidak pernah flag tersimpan yang bisa lupa dibersihkan endpoint ini. Heartbeat server `decommissioned` diakui dan dicatat tetapi tidak pernah membalikkan statusnya kembali ke `online`.                                                                                                                                                                                                                                                                                                                                                                                  |
| Referensi lintas-tenant lewat tabel nonce/results                                                          | FK komposit `(tenant_id, worker_id)` pada tabel nonce / `(tenant_id, server_id, idempotency_key)` pada tabel results (pemeriksaan RI berjalan melewati RLS — GHSA-r7cx-c4jh-cvvw), bukan FK kolom tunggal biasa, ditambah RLS `FORCE` pada kedua tabel baru.                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| Volume request tak terbatas/abusif terhadap permukaan tak terautentikasi                                   | Body request dibatasi (`readCappedText`, batas ukuran streaming sama yang dipakai `readJsonBody`), ditambah DUA lapis rate limit pada keempat rute: per-(tenant, worker) sesuai klaim di body (membatasi penyalahgunaan satu identitas nyata, tapi direset oleh penyerang yang sekadar mengganti identitas yang diklaim tiap request — keterbatasan yang didokumentasikan jujur, bukan celah diam-diam) dan per-IP sumber (`resolveClientIp`, batas volume agregat yang sesungguhnya, lebih longgar agar toleran terhadap NAT/host bersama yang menjalankan beberapa proses worker).                                                                                                                      |

Kesenjangan yang diketahui, ditandai bukan disiasati diam-diam: pull worker referensi sisi-OMES yang di-merge untuk `ahliweb/omes#192` belum membangkitkan keypair Ed25519 sungguhan atau mengirim header signature/nonce/timestamp apa pun. Modul ini tidak melonggarkan verifikasi agar cocok dengan stub itu.

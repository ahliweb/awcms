...
| `TRUSTED_PROXY_ENABLED`                     | Ya (produksi)  | `false`                                  | –        | Wajib eksplisit di produksi. `true` bila di belakang proxy yang MENIMPA `X-Forwarded-For` (profil nginx); `false` bila terekspos langsung. Salah pilih merusak rate limit login ke dua arah — lihat catatan di bawah |
